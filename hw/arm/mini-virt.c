...
static const CPUArchIdList *virt_possible_cpu_arch_ids(MachineState *ms)
{
    ...
    for (int i = 0; i < ms->possible_cpus->len; i++) {
        ...
        ms->possible_cpus->cpus[i].arch_id = arm_build_mp_affinity(i, GICV3_TARGETLIST_BITS);
    }
    return ms->possible_cpus;
}
...
static void virt_machine_done(Notifier *notifier, void *data)
{
    ...
    int ret = arm_load_dtb(info->dtb_start, info, info->dtb_limit, as, ms, cpu);
    assert(ret > 0);
}
...
static void mini_virt_machine_init(void)
{
    type_register_static(&mini_virt_machine_info);
}

type_init(mini_virt_machine_init);
